...
class MaskGuidanceModule(nn.Module):
    def __init__(self, dim, img_size=224, stage_resolution=14):
        super().__init__()
        ...
        self.mask_conv = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.AdaptiveAvgPool2d((stage_resolution, stage_resolution)),
        ...
        self.delta_conv = nn.Conv2d(dim, dim, kernel_size=1, stride=1, padding=0, bias=True)
        nn.init.constant_(self.delta_conv.weight, 0.0)
        nn.init.constant_(self.delta_conv.bias, 0.0)
    ...
    def forward(self, features, mask):
        ...
        if mask.shape[2:] != (self.img_size, self.img_size):
            print(f"Warning: Expected mask size ({self.img_size}, {self.img_size}), got {mask.shape[2:]}")

            mask = F.interpolate(mask, size=(self.img_size, self.img_size),
                            mode='bilinear', align_corners=False)
        if mask.max() > 1.0 or mask.min() < 0.0:
            print(f"Warning: Mask values out of range [0,1]: [{mask.min():.3f}, {mask.max():.3f}]")
            mask = torch.clamp(mask, 0.0, 1.0)
        features_spatial = features.transpose(-2, -1).contiguous().view(B, C, H, W)
        mask_features = self.mask_conv(mask)
        combined_features = torch.cat([features_spatial, mask_features], dim=1)
        fused_features = self.fusion_conv(combined_features)
        attention_weights = self.attention(fused_features)
        self.last_attention_weights = attention_weights.detach()
        delta = self.delta_conv(attention_weights)
        enhanced_features = features_spatial + delta
        enhanced_features = self.dropout(enhanced_features)
        enhanced_features = enhanced_features.view(B, C, -1).transpose(-2, -1).contiguous()
...
class CSWinTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=96, depth=[2,2,6,2], split_size = [3,5,7],
                ...
                drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm, use_chk=False, use_mask_guidance=True):
        super().__init__()
        ...
        if self.use_mask_guidance:
            self.mask_guidance = MaskGuidanceModule(
                dim=curr_dim,
                img_size=img_size,
                stage_resolution=img_size//16
            )
    ...
    def forward_features(self, x, mask=None):
        ...
        for blk in self.stage3:
            if self.use_chk and self.training:
                ...
                x = blk(x)
        if self.use_mask_guidance and mask is not None:
            x = self.mask_guidance(x, mask)
        x = self.merge3(x)
